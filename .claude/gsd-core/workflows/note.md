...
- **Global scope**: `/workspaces/fabric-gsd-demo/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/workspaces/fabric-gsd-demo/.claude/notes/`)
...
2. Glob `/workspaces/fabric-gsd-demo/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/workspaces/fabric-gsd-demo/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/workspaces/fabric-gsd-demo/.claude/notes/` — works in any directory
